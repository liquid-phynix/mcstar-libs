...
void _main(int pnum, int dnum){
    ...
    cl::Buffer buffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, LIST_SIZE, host_ptr);
    ...
    { // warm-up
        ...
        int* host_ptr_2 = (int*)queue.enqueueMapBuffer(buffer, CL_FALSE, CL_MAP_READ | CL_MAP_WRITE, 0, LIST_SIZE);
        ...
        queue.finish();
        if(host_ptr != host_ptr_2){
            std::cerr << "host and mapped pointers differ" << std::endl;
        }
    }
    ...
    for(int exp = start_exp; exp >= min_exp; exp--){
        ...
        for(cl_ulong iter = 0; iter < iters; iter++){
            ...
            //queue.enqueueMapBuffer(buffer, CL_TRUE, CL_MAP_WRITE_INVALIDATE_REGION, 0, LIST_SIZE, NULL, &map_1_buffer);
            host_write.start();
            for(int i = 0; i < len; i++){
            ...
            //queue.enqueueMapBuffer(buffer, CL_TRUE, CL_MAP_READ, 0, LIST_SIZE, NULL, &map_2_buffer);
