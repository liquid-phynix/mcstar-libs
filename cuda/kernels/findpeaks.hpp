...
__global__ void kernel_findpeaks(Float* arr, int3 rdims, Maximum* host_arr){
    IDX012(rdims);
...
void call_kernel_findpeaks(GPUArray& input, std::string peaks_outfn, std::string oorder_outfn, Float3 hh, Float3 len){
    if(len.x != len.y or len.x != len.z){
        std::cerr << "orientattional mapping needs a cube" << std::endl;
        return;
    }
    int elems = input.real_elems();
    Maximum* host_ptr;
    CUERR(cudaHostAlloc((void**)&host_ptr, elems * sizeof(Maximum), cudaHostAllocDefault));
    int num_of_maxima= 0;
    Launch l(input.real_vext());
    CUERR(cudaMemcpyToSymbol(gpu_maxlist_len, &num_of_maxima, sizeof(int)));
    kernel_findpeaks<<<l.get_gs(), l.get_bs()>>>(input.ptr_real(), input.real_vext(), host_ptr);
    ...
    if(outfile != NULL){
        for(int i = 0; i < num_of_maxima; i++){
            Maximum& m = host_ptr[i];
            float x0 = m.i0 * hh.x, x1 = m.i1 * hh.y, x2 = m.i2 * hh.z;
            fprintf(outfile, "%f %f %f %f\n", x0, x1, x2, m.field);
        }
        fclose(outfile);
    }
    bond_order_analysis(host_ptr, num_of_maxima, hh.x, hh.y, hh.z, oorder_outfn, 0.5, len.x, 10);
